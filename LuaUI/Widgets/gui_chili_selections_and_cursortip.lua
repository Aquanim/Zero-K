...
-- Probably can't be autogenerated easily since bomberprec's weapons are dreadful
local burstUnitsDefs = {
	[UnitDefNames["cloaksnipe"].id] = {burst = 1500},
	[UnitDefNames["vehheavyarty"].id] = {burst = 800},
	[UnitDefNames["bomberprec"].id] = {burst = 800},
	[UnitDefNames["bomberheavy"].id] = {burst = 2000},
	[UnitDefNames["hoverskirm"].id] = {burst = 310*2},
	[UnitDefNames["hoverarty"].id] = {burst = 3000},
	[UnitDefNames["turretantiheavy"].id] = {burst = 4000},
	[UnitDefNames["tacnuke"].id] = {burst = 3500},
}
	...
	local function UpdateDynamicGroupInfo()
		...
		for i = 1, total_count do
			...
			if ud then
				...
				if burstUnitsDefs[ud] and burstUnitsDefs[ud].burst then
					total_burst = total_burst + burstUnitsDefs[ud].burst
				end
		...
		if total_burst ~= 0 then
			unitInfoString = unitInfoString ..
				WG.Translate("interface", "burst_damage") .. ": " .. Format(total_burst) .. "\n"
		end
